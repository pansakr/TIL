...
* 일대다 단방향, 다대다 연관관계는 실제 업무에서 거의 사용하지 않는다
...
* 다대일
    ...
    - 다대일 단방향
        ...
        - 회원은 하나의 팀을 가지고, 팀은 여러 회원을 가질 때 회원 입장에서 팀을 바라본 관계. 다(회원) 대 일(팀) 이다
    ...
    - 일대다 단방향
        ...
        - 테이블의 일대다 관계는 항상 다 쪽에 외래키가 있는데, jpa에서 일대다 설정 시 일 쪽이 외래키를 가진다
            ...
            - 이대로 매핑할 시 회원 테이블의 외래키를 팀 객체에서 관리하게 되는 특이한 구조가 된다
                ...
                - 그리고 위 상황에서 team 엔티티 persist() 시 team, member의 insert sql 에 더해 member의 update sql 이 추가로 생성되었는데 이것 자체로 문제가 된다

                    - 예시는 간단해서 그렇지 여러 테이블이 복잡하게 얽혀 있는 실제 업무에서는 위와 같은 상황이 발생하면 왜 건드리지 않은 member의 update sql이 생성되었는지 파악하기 어렵다
