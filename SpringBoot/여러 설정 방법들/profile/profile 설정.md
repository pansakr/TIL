...
* 실행 시 실행아이콘 왼쪽의 xxxApplication -> edit configurations -> vm Option에 -Dspring.profiles.active="프로파일 이름" 을 설정하면 해당 프로파일의 yml설정을 읽어 실행한다

* yml에 server.servlet.context-path 옵션은 루트 경로를 의미한 것으로 보통은 /로 하지만 실행 환경을 명확하게 확인할 수 있게 최상위 경로로 해당 실행환경 이름을 지정할 수 있다
...
spring:
  profiles:
    active: local  // 실행 시 프로파일이 명시적으로 지정되지 않으면 'local' 을 기본으로 사용
