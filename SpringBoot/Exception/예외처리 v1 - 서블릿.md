...
* WAS 까지 예외가 도달하면 WAS 는 서버 내부에서 처리할 수 없는 오류가 발생한 것으로 보고 HTTP 상태 코드 500을 반환한다
...
* 예외가 발생했을 때 HttpServletResponse 가 제공하는 sendError 라는 메서드를 사용해 WAS 에 예외가 발생했다는 것을 전달할 수 있다

* 서블릿 컨테이너는(was) 고객에게 응답 전에 response 에 sendError() 가 호출되었는지 확인한다

* 만약 호출되었다면 설정한 예외 코드에 맞추어 기본 오류 페이지를 보여준다

```java
@Controller
public class ServletExController {

    // 서블릿 컨테이너에 404 예외가 발생했다는것과 예외 메시지 전달. 서블릿 컨테이너는 기본 404 예외 페이지 보여줌
...
### 서블릿 - 웹 애플리케이션(was) 예외처리 - 예외 화면 제공
...
* 서블릿은 Exception (예외)가 발생해서 서블릿 밖으로 전달되거나 response.sendError() 가 호출 되었을 때 설정된 예외 페이지를 찾는다

```
// 예외 발생 시 흐름
컨트롤러(예외발생) -> 인터셉터 -> 서블릿 -> 필터 -> WAS

// sendError() 로 예외 전달 시 흐름
컨트롤러(response.sendError()) -> 인터셉터 -> 서블릿 -> 필터 -> WAS(sendError 호출 기록 확인)
```

* WAS는 해당 예외를 처리하는 예외 페이지 정보를 확인한다
```
...
```
// 예외 처리 순서
1. RuntimeException 예외가 was까지 전달되면 was는 예외 페이지 정보를 확인한다  
2. 확인해보니 RuntimeException 의 예외 페이지로 /error-page/500 이 지정되어 있다
3. WAS는 예외 페이지를 출력하기 위해 /error-page/500 를 다시 요청한다
...
// 예외 정보 필요 시 이런 식으로 사용할 수 있다
log.info("ERROR_STATUS_CODE=", request.getAttribute(RequestDispatcher.ERROR_STATUS_CODE));
```
* 클라이언트의 추가 요청 없이 오직 서버 내부에서 예외 페이지를 찾기 위해 추가적인 호출을 한다

* 그런데 필터나 인터셉터 같은 경우는 주로 인증에 사용되는데, 예외 페이지를 호출한다고 해당 필터, 인터셉터를 또 호출하는것은 비효율적이다

* 그래서 클라이언트로부터 발생한 정상 요청인지, 예외 페이지를 출력하기 위한 내부 요청인지 구분할 수 있어야 한다
...
public class WebConfig implements WebMvcConfigurer {
    ...
    public void addInterceptors(InterceptorRegistry registry) {
        ...
        // 인터셉터를 호출하지 않을 url 
        .excludePathPatterns("/css/**", "/*.ico" , "/error", "/error-page/**" // 예외 페이지 경로);
}
```
