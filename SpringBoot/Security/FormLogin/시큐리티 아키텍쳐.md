...
* AbstractAuthenticationProcessingFilter(추상 클래그) 를 구현한 UsernamePasswordAuthenticationFilter 가 요청을 가로챈다

  - 그림에는 AuthenticationFilter 가 있지만, 이는 인증 요청을 가로채는 필터들을 통칭하는 개념적 표현으로, 실제로는 존재하지 않는다
 
    - 실제로는 AbstractAuthenticationProcessingFilter 계열의 필터들이 요청을 가로챈다
