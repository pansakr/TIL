...
* 연관된 스레드들을 그룹으로 묶기 위해 사용

* 스레드 그룹이 다른 스레드 그룹에 포함될 수 있다

* 스레드를 일괄적으로 다루거나 보안상 분리하기 위해 사용
                ...
                if (line.length() == 1) {
                    ...
                    if ("abc".contains(line)) {
                        ...
                        System.out.printf(
                                "%s : %d / %d%n",
                                group.getName(),
                                group.activeCount(), // 스레드 개수
                                //  내부의 스레드들이 멈춰도 active로 카운트
                                group.activeGroupCount() // 스레드 내부 그룹 개수
                        );
                    }
...
* 주 스레드의 작업이 끝나면 자동 종료

* 스레드 변수.setDaemon(true) 으로 특정 스레드의 데몬 스레드로 지정해 주어야 한다
